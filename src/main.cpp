...
#define INKAY_VERSION "v3.1.0-protarium-mh3u-bo2-r46"
...
WUMS_APPLICATION_STARTS() {
    ...
    // Re-arm the BO2 auth patch for this launch (module persists across apps).
    reset_bo2_auth_patch();
}
...
WUMS_ALL_APPLICATION_STARTS_DONE() {
    ...
    setup_olv_libs();
    // BO2's multiplayer RPL isn't loaded yet here; the real patch is applied
    // lazily from the DNS hook when a Demonware host is resolved. This call is
    // a harmless no-op safety net (skips if the RPL isn't present).
    patch_bo2_auth();
    peertopeer_patch();
